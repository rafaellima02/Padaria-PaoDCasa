...
export async function PUT(request: Request) {
    try {
      ...
      const tipoEnderecoExistente = await prisma.endereco.findUnique({
        where: {
          id: id,
        },
      });
  
      if (!tipoEnderecoExistente) {
        return new NextResponse("Tipo de usuário não encontrado", { status: 404 });
      }
  
      // Atualize a inspeção com os novos dados
      const updateEndereco = await prisma.endereco.update({
        where: {
          id: id,
        },
        data: {
